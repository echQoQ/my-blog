...
<span class="line"><span>        -rm *.mod.c *.o *.order *.symvers *.ko *.mod</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><h5 id="_1-2-2实验过程及结果" tabindex="-1"><a class="header-anchor" href="#_1-2-2实验过程及结果"><span>1.2.2实验过程及结果</span></a></h5><ol><li><code>make</code>编译</li><li><code>insmod stuldThread.ko stu_id=&quot;22920212204174&quot;</code>将模块导入内核并输入学号</li><li><code>lsmod</code>查看导入情况<img src="`+l+'" alt="" loading="lazy"></li><li><code>dmesg | tail -n 25</code> 查看内核消息队列<img src="'+p+'" alt="" loading="lazy"></li></ol><h2 id="_2-任务二-绑定内核线程到指定cpu" tabindex="-1"><a class="header-anchor" href="#_2-任务二-绑定内核线程到指定cpu"><span>2 任务二：绑定内核线程到指定CPU</span></a></h2><h4 id="_2-1-基本知识" tabindex="-1"><a class="header-anchor" href="#_2-1-基本知识"><span>2.1 基本知识</span></a></h4><ul><li><code>kthread_bind()</code><ul><li><strong>参数</strong>： <ul><li><code>k</code>：指向<code>task_struct</code>的指针，代表要绑定的线程。</li><li><code>cpu</code>：整数，表示CPU的编号，线程将被绑定到这个CPU上。</li></ul></li><li><strong>作用</strong>： <ul><li>将指定的线程绑定到特定的CPU，以确保线程总是在该CPU上运行。</li></ul></li></ul></li></ul><h3 id="_2-2-任务2-1" tabindex="-1"><a class="header-anchor" href="#_2-2-任务2-1"><span>2.2 任务2.1</span></a></h3><blockquote><p>你知道MyPrintk中current全局变量的含义吗？请你编写kthread_bind_test.c，通过实验判断将线程绑定到指定CPU核心时，线程应当处于什么状态？唤醒线程后能否通过kthread_bind()切换线程所在CPU？ 通过命令查看当前机器的CPU核数，若在绑定时设定的CPU核心ID超过机器本身的CPU核数，会产生什 么结果？请结合实验结果验证你的结论。</p></blockquote><ul><li>根据示例中MyPrink代码推断，current全局变量应该是指运行中的线程本身对应的指针</li><li>当将线程唤醒之后不能通过<code>kthread_bind</code>切换线程所在CPU，验证代码如下<img src="'+d+'" alt="" loading="lazy">其他与示例一致</li><li>实验结果如下]]<img src="'+c+'" alt="" loading="lazy"></li><li>可以看见出现了报错，且线程并未切换到指定的1号，可见前面的结论是正确的</li></ul><hr><ul><li>将线程绑定到指定CPU核心时，线程应当处于什么状态，下面也将通过实验求证，代码如下<img src="'+t+'" alt="" loading="lazy"></li><li>实验结果为<img src="'+r+'" alt="" loading="lazy"></li><li>线程状态为2，代表<code>TASK_UNINTERRUPTIBLE</code>，这意味着线程正在等待某个特定条件，且不能被信号中断。</li></ul><hr><ul><li>通过命令查看当前机器的CPU核数 <ul><li><code>nproc</code><img src="'+o+'" alt="" loading="lazy"></li></ul></li><li>若在绑定时设定的CPU核心ID超过机器本身的CPU核数<img src="'+u+`" alt="" loading="lazy"></li><li>可见未切换到指定的5号，则说明ID超过机器本身CPU核数时切换不成功</li></ul><h4 id="_2-3-任务2-2" tabindex="-1"><a class="header-anchor" href="#_2-3-任务2-2"><span>2.3 任务2.2</span></a></h4><blockquote><p>假设当前服务器CPU的核数为N，请你编写<code>kthread_bind_cores.c</code>，实现创建N个线程，每个线程与一个CPU核心绑定，并在各个线程运行时每隔2秒打印一次当前线程名和占用的CPU ID，要求每个线程使用同一个MyPrintk()打印函数。</p></blockquote><ol><li>代码</li></ol><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" data-title="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>#include &lt;linux/module.h&gt;</span></span>
...
<span class="line"><span>.PHONY: clean</span></span>
<span class="line"><span>clean:</span></span>
<span class="line"><span>	-rm *.mod.c *.o *.order *.symvers *.ko *.mod</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div><div class="line-number"></div></div></div><ol start="2"><li>编译运行</li></ol><figure><img src="`+g+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><figure><img src="'+h+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><h3 id="_5-任务五-使用cgroup限制cpu核数" tabindex="-1"><a class="header-anchor" href="#_5-任务五-使用cgroup限制cpu核数"><span>5 任务五：使用cgroup限制CPU核数</span></a></h3><ol><li>基本知识——cgroup</li></ol><blockquote><p>cgroup (Control Groups)是Linux中对任意线程进行分组化管理的工具。</p></blockquote><ol start="2"><li>复现实验流程，实现对进程使用CPU核数的限制</li></ol><p>运行截图： <img src="'+_+'" alt="" loading="lazy"><img src="'+k+'" alt="" loading="lazy"></p><h3 id="_6-任务六-使用cgroup限制cpu利用率" tabindex="-1"><a class="header-anchor" href="#_6-任务六-使用cgroup限制cpu利用率"><span>6 任务六：使用cgroup限制CPU利用率</span></a></h3><ol start="2"><li>复现实验流程 <img src="'+y+'" alt="" loading="lazy"><img src="'+f+'" alt="" loading="lazy"><img src="'+E+`" alt="" loading="lazy"></li></ol><p>可见经过设置CPU利用限制，进程<code>cgroup_cpu</code>的CPU利用率下降至19.9%</p><p><code>cpu.cfs_quota_us</code> 和 <code>cpu.cfs_period_us</code> 是 Linux 内核中控制 CFS (Completely Fair Scheduler) CPU 配额的两个参数。CFS 是 Linux 内核中的一种调度器，负责在多个进程之间分配 CPU 时间。</p><p>这两个参数的含义如下：</p><ol><li><code>cpu.cfs_quota_us</code>：这个参数定义了在一段时间内一个 cgroup 可以使用 CPU 的总时间量。单位是微秒（μs）。例如，如果 <code>cpu.cfs_quota_us</code> 设置为 100000，那么表示在 <code>cpu.cfs_period_us</code> 定义的时间内（通常是1秒），这个 cgroup 可以使用 CPU 100毫秒。如果设置为 -1，则表示没有限制。</li><li><code>cpu.cfs_period_us</code>：这个参数定义了一个周期的长度，用于计算 <code>cpu.cfs_quota_us</code> 中定义的 CPU 时间量。单位也是微秒（μs）。例如，如果 <code>cpu.cfs_period_us</code> 设置为 1000000（即1秒），而 <code>cpu.cfs_quota_us</code> 设置为 500000（即0.5秒），那么这个 cgroup 在每秒的时间内可以使用 CPU 50%。</li></ol><p>运行如下指令使cgroup_cpu的利用率维持在40%</p><div class="language- line-numbers-mode" data-highlighter="shiki" data-ext="" data-title="" style="--shiki-light:#383A42;--shiki-dark:#abb2bf;--shiki-light-bg:#FAFAFA;--shiki-dark-bg:#282c34;"><pre class="shiki shiki-themes one-light one-dark-pro vp-code"><code><span class="line"><span>echo 40000 &gt; /sys/fs/cgroup/cpu/mycpu/cpu.cfs_quota_us </span></span>
<span class="line"><span>echo 486 &gt; /sys/fs/cgroup/cpu/mycpu/tasks</span></span></code></pre><div class="line-numbers" aria-hidden="true" style="counter-reset:line-number 0;"><div class="line-number"></div><div class="line-number"></div></div></div><figure><img src="`+q+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure>',76)]))}const N=s(x,[["render",A],["__file","实验二 openEuler 操作系统 进程管理实验.html.vue"]]),U=JSON.parse('{"path":"/reports/%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/%E5%AE%9E%E9%AA%8C%E4%BA%8C%20openEuler%20%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F%20%E8%BF%9B%E7%A8%8B%E7%AE%A1%E7%90%86%E5%AE%9E%E9%AA%8C.html","title":"实验二 openEuler 操作系统 进程管理实验","lang":"zh-CN","frontmatter":{"title":"实验二 openEuler 操作系统 进程管理实验","cover":"/assets/images/cover2.jpg","icon":"paper-plane","order":2,"author":"Mr.Liu","category":["实验报告"],"tag":["操作系统","实验报告"],"sticky":false,"star":false,"footer":"箱根山岳险天下","copyright":"无版权","description":"[! col2] 实验人 姓名：刘志豪 学号：22920212204174 0x01 实验目的 ⚫ 学习掌握Linux内核线程的创建； ⚫ 学习掌握Linux内核线程的状态转换； ⚫ 了解如何通过/proc文件系统获取系统当前运行状态； ⚫ 了解cgroup进程分组化管理工具，学习如何限制内核线程的CPU核心数和利用率。 0x02 实验任务 1 任务一...","head":[["meta",{"property":"og:url","content":"https://echqoq.github.io/my-blog/my-blog/reports/%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F/%E5%AE%9E%E9%AA%8C%E4%BA%8C%20openEuler%20%E6%93%8D%E4%BD%9C%E7%B3%BB%E7%BB%9F%20%E8%BF%9B%E7%A8%8B%E7%AE%A1%E7%90%86%E5%AE%9E%E9%AA%8C.html"}],["meta",{"property":"og:site_name","content":"Mr.Liu"}],["meta",{"property":"og:title","content":"实验二 openEuler 操作系统 进程管理实验"}],["meta",{"property":"og:description","content":"[! col2] 实验人 姓名：刘志豪 学号：22920212204174 0x01 实验目的 ⚫ 学习掌握Linux内核线程的创建； ⚫ 学习掌握Linux内核线程的状态转换； ⚫ 了解如何通过/proc文件系统获取系统当前运行状态； ⚫ 了解cgroup进程分组化管理工具，学习如何限制内核线程的CPU核心数和利用率。 0x02 实验任务 1 任务一..."}],["meta",{"property":"og:type","content":"article"}],["meta",{"property":"og:image","content":"https://echqoq.github.io/my-blog/my-blog/assets/images/cover2.jpg"}],["meta",{"property":"og:locale","content":"zh-CN"}],["meta",{"property":"og:updated_time","content":"2024-09-03T05:45:17.000Z"}],["meta",{"name":"twitter:card","content":"summary_large_image"}],["meta",{"name":"twitter:image:src","content":"https://echqoq.github.io/my-blog/my-blog/assets/images/cover2.jpg"}],["meta",{"name":"twitter:image:alt","content":"实验二 openEuler 操作系统 进程管理实验"}],["meta",{"property":"article:author","content":"Mr.Liu"}],["meta",{"property":"article:tag","content":"操作系统"}],["meta",{"property":"article:tag","content":"实验报告"}],["meta",{"property":"article:modified_time","content":"2024-09-03T05:45:17.000Z"}],["script",{"type":"application/ld+json"},"{\\"@context\\":\\"https://schema.org\\",\\"@type\\":\\"Article\\",\\"headline\\":\\"实验二 openEuler 操作系统 进程管理实验\\",\\"image\\":[\\"https://echqoq.github.io/my-blog/my-blog/images/20240322171539.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322171842.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322181311.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322180700.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322181804.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322181727.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322182154.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240322182418.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405193518.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405193117.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405194201.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405212713.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405212329.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405213841.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405213906.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405214743.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405214827.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405214847.png\\",\\"https://echqoq.github.io/my-blog/my-blog/images/20240405215201.png\\"],\\"dateModified\\":\\"2024-09-03T05:45:17.000Z\\",\\"author\\":[{\\"@type\\":\\"Person\\",\\"name\\":\\"Mr.Liu\\"}]}"]]},"headers":[{"level":2,"title":"0x01 实验目的","slug":"_0x01-实验目的","link":"#_0x01-实验目的","children":[]},{"level":2,"title":"0x02 实验任务","slug":"_0x02-实验任务","link":"#_0x02-实验任务","children":[{"level":3,"title":"1 任务一：创建并运行内核线程","slug":"_1-任务一-创建并运行内核线程","link":"#_1-任务一-创建并运行内核线程","children":[]}]},{"level":2,"title":"2 任务二：绑定内核线程到指定CPU","slug":"_2-任务二-绑定内核线程到指定cpu","link":"#_2-任务二-绑定内核线程到指定cpu","children":[{"level":3,"title":"2.2 任务2.1","slug":"_2-2-任务2-1","link":"#_2-2-任务2-1","children":[]},{"level":3,"title":"2.4 任务2.3","slug":"_2-4-任务2-3","link":"#_2-4-任务2-3","children":[]},{"level":3,"title":"3 任务三：内核线程的睡眠和唤醒","slug":"_3-任务三-内核线程的睡眠和唤醒","link":"#_3-任务三-内核线程的睡眠和唤醒","children":[]},{"level":3,"title":"4 任务四：利用/proc文件系统实时获取系统状态信息","slug":"_4-任务四-利用-proc文件系统实时获取系统状态信息","link":"#_4-任务四-利用-proc文件系统实时获取系统状态信息","children":[]},{"level":3,"title":"5 任务五：使用cgroup限制CPU核数","slug":"_5-任务五-使用cgroup限制cpu核数","link":"#_5-任务五-使用cgroup限制cpu核数","children":[]},{"level":3,"title":"6 任务六：使用cgroup限制CPU利用率","slug":"_6-任务六-使用cgroup限制cpu利用率","link":"#_6-任务六-使用cgroup限制cpu利用率","children":[]}]}],"git":{"createdTime":1725342317000,"updatedTime":1725342317000,"contributors":[{"name":"Iwindy","email":"12398041+iwindy0@user.noreply.gitee.com","commits":1}]},"readingTime":{"minutes":10.49,"words":3148},"filePathRelative":"reports/操作系统/实验二 openEuler 操作系统 进程管理实验.md","localizedDate":"2024年9月3日","excerpt":"<blockquote>\\n<p>[! col2] 实验人</p>\\n<ul>\\n<li>姓名：刘志豪</li>\\n<li>学号：22920212204174</li>\\n</ul>\\n</blockquote>\\n<h2>0x01 实验目的</h2>\\n<p>⚫ 学习掌握Linux内核线程的创建；</p>\\n<p>⚫ 学习掌握Linux内核线程的状态转换；</p>\\n<p>⚫ 了解如何通过/proc文件系统获取系统当前运行状态；</p>\\n<p>⚫ 了解cgroup进程分组化管理工具，学习如何限制内核线程的CPU核心数和利用率。</p>\\n<h2>0x02 实验任务</h2>\\n<h3>1 任务一：创建并运行内核线程</h3>","autoDesc":true}');export{N as comp,U as data};
